...
## Subroutines    ##
####################
def computeAffinity(Avec, K, T, expr, nCons, KINorPT):
    nSites = T.shape[0]    
    A = np.zeros_like(T)
    (x, y) = np.where(T != 0)
    #K2 = K
    #for j in range(0, nCons):
    #    for k in range(0, len(KINorPT)):
    #        K2[k, j] = K[k, j] * KINorPT[k]
...
def computeAffinityGrad(Avec, K, T, expr, nCons, KINorPT):
    nSites = T.shape[0]
    nKins = T.shape[1]
    A = np.zeros_like(T)
    #A2 = np.zeros_like(T)
    ...
    for i in range(0, nSites):
        for j in range(0, nKins):
            ...
            for l in range(0, nCons):
                if A[i, j] != 0:
                    g = g + (2 * ((K2[j, l] * A[i, j]) / sum(A[i, :]) - expr[i, l]) *
                             ((K2[j, l] * sum(A[i, :]) - sum(K[:, l] * A[i, :])) / (sum(A[i, :]) ** 2))) 
            if A[i, j] != 0:
                grad[i] = g    
    
    return(grad)

def computeActivity(K, A, T, expr, KINorPT):
    Avec = A
    nSites = T.shape[0]    
    A = np.zeros_like(T)
    J = 0
...
def computeActivityGrad(K, A, T, expr, KINorPT):
    Avec = A
    nSites = T.shape[0]
    nKins = T.shape[1]
    A = np.zeros_like(T)
...
def fitActivities(T, expr, nCons, nIters, KINorPT):
    ...
    for i in range(0, nIters):
        ...
        while (J1 - J2 > 10):
            ...
            res = minimize(computeAffinity, A, method = "SLSQP", jac = computeAffinityGrad, 
                           args = (K, T, expr, nCons, KINorPT), bounds = bndsA, options = {'disp': True})
            J1 = res.fun
            A = res.x
            As[:, i] = A
            ...
            for j in range(0, nCons):
                ...
                res = minimize(computeActivity, K[:, j], method = "SLSQP", jac = computeActivityGrad,
                                args = (A, T, expr[:, j], KINorPT), bounds = bndsK, options = {'disp': True})
                K[:, j] = res.x
                J2 = J2 + res.fun
    ...
    J = np.zeros((nIters))
    for i in range(0, nIters):
        J[i] = computeAffinity(As[:, i], Ks[i, :, :], T, expr, nCons, KINorPT)
...
for i in range(0, len(sitePTase)):
    iPTases = (sitePTase[i].replace('"', '')).split(",")
    for iPTase in iPTases:
        ptase.append(iPTase)

kins = sorted(set(kins))
ptase = sorted(set(ptase))

## Create a truth table (T = [nSites x nKins])
nSites = len(sites)
nKins = len(kins)
T = np.zeros((nSites, nKins))
KINorPT = np.zeros(nKins)
for i in range(0, nSites):
    iKins = (siteKins[i].replace('"', '')).split(",")
...
for i in range(0, nRuns):
    ...
    nIters = 1
    #nIters = 1
    (iA, iK) = fitActivities(T, expr, nCons, nIters, KINorPT)
    As[:, i] = iA
